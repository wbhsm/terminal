...
// TODO CARLOS: generated .h files are not being populated properly
// defaults.h is a file containing the default json settings in a std::string_view
#include "defaults1.h"
#include "defaults-universal1.h"
// userDefault.h is like the above, but with a default template for the user's settings.json.
#include "userDefaults1.h"
...
// Method Description:
// - Creates a AppSettings from whatever's saved on disk, or instantiates
...
// Return Value:
// - a unique_ptr containing a new AppSettings object.
winrt::com_ptr<AppSettings> AppSettings::LoadAll()
{
    auto resultPtr = LoadDefaults();

    // TODO CARLOS: TraceLogging
    ...
    //const auto hardcodedDefaultGuid = resultPtr->Globals().UnparsedDefaultProfile();
    ...
    if (!fileHasData)
    {
        // TODO CARLOS: ApplyFirstRunChangesToSettingsTemplate
        ...
        auto userSettings{ UserSettingsJson };
...
// - <none>
// Return Value:
// - a unique_ptr to a AppSettings with the connection types and settings for Universal terminal
winrt::com_ptr<AppSettings> AppSettings::LoadUniversal()
{
    ...
    auto resultPtr = winrt::make_self<implementation::AppSettings>();
...
// Function Description:
// - Creates a new AppSettings object initialized with settings from the
//   hardcoded defaults.json.
// Arguments:
// - <none>
// Return Value:
// - a unique_ptr to a AppSettings with the settings from defaults.json
winrt::com_ptr<AppSettings> AppSettings::LoadDefaults()
{
    auto resultPtr = winrt::make_self<implementation::AppSettings>();
...
// Return Value:
// - <none>
void AppSettings::_LoadDynamicProfiles()
{
    std::unordered_set<std::wstring> ignoredNamespaces;
    const auto disabledProfileSources = AppSettings::_GetDisabledProfileSourcesJsonObject(_userSettings);
    ...
    for (auto& generator : _profileGenerators)
    {
        ...
        else
        {
            try
            {
                ...
                {
                    // TODO CARLOS: MIDL5037: [msg]Invalid parameter to IReference or IReferenceArray (value in get_X/put_X) for Profiles.Source
...
// Return Value:
// - <none>
void AppSettings::_ParseJsonString(std::string_view fileData, const bool isDefaultSettings)
{
    // Ignore UTF-8 BOM
...
bool AppSettings::_PrependSchemaDirective()
{
...
// Return Value:
// - true iff we've made changes to the _userSettingsString that should be persisted.
bool AppSettings::_AppendDynamicProfilesToUserSettings()
{
    ...
    for (const auto& profileProj : _profiles)
    {
        const auto profile = winrt::get_self<implementation::Profile>(profileProj);
...
// Arguments:
// - json: an object which should be a serialization of a AppSettings object.
// Return Value:
// - a new AppSettings instance created from the values in `json`
std::unique_ptr<AppSettings> AppSettings::FromJson(const Json::Value& json)
{
    auto resultPtr = std::make_unique<AppSettings>();
...
//   be replaced.
// Arguments:
// - json: an object which should be a partial serialization of a AppSettings object.
// Return Value:
// <none>
void AppSettings::LayerJson(const Json::Value& json)
{
    auto globals = winrt::get_self<implementation::GlobalSettings>(_globals);
    ...
    // TODO CARLOS: might need to save globals back to _globals
    //_globals = winrt::get_abi<GlobalSettings>(globals);
}
...
// Return Value:
// - <none>
void AppSettings::_LayerOrCreateProfile(const Json::Value& profileJson)
{
    ...
    else
    {
        ...
        {
            winrt::com_ptr<implementation::Profile> profile{};
...
//   profile exists.
Profile* AppSettings::_FindMatchingProfile(const Json::Value& profileJson)
{
    for (uint32_t i = 0; i < _profiles.Size(); ++i)
    //for (auto& profile : _profiles)
    {
        auto profileProj = _profiles.GetAt(i);
...
// Return Value:
// - <none>
void AppSettings::_ApplyDefaultsFromUserSettings()
{
...
// Return Value:
// - <none>
void AppSettings::_LayerOrCreateColorScheme(const Json::Value& schemeJson)
{
    // Layer the json on top of an existing profile, if we have one:
    ...
    {
        // TODO REVIEWER: Check that ownership is handled properly here (probably isn't)
        auto scheme = ColorScheme::FromJson(schemeJson);
        _schemes.Insert(scheme->Name(), *scheme.get());
    }
}
...
//   color scheme exists.
ColorScheme* AppSettings::_FindMatchingColorScheme(const Json::Value& schemeJson)
{
    if (auto schemeName = ColorScheme::GetNameFromJson(schemeJson))
    {
        if (auto result = _schemes.TryLookup(schemeName->c_str()))
        {
            return winrt::get_self<implementation::ColorScheme>(result);
        }
    }
    return nullptr;
}
...
void AppSettings::_WriteSettings(const std::string_view content)
{
    auto pathToSettingsFile{ AppSettings::GetSettingsPath() };
...
std::optional<std::string> AppSettings::_ReadUserSettings()
{
    const auto pathToSettingsFile{ AppSettings::GetSettingsPath() };
...
std::optional<std::string> AppSettings::_ReadFile(HANDLE hFile)
{
...
std::filesystem::path AppSettings::GetDefaultSettingsPath()
{
...
const Json::Value& AppSettings::_GetProfilesJsonObject(const Json::Value& json)
{
...
//   given object
const Json::Value& AppSettings::_GetDisabledProfileSourcesJsonObject(const Json::Value& json)
{
    if (!json)
    {
